...
import { ImageWithFallback, GlobeTrotterLogo } from "../components/UI";
export default function Login({ mode }) {
  const signup = mode === "signup",
    reset = mode === "reset",
    nav = useNavigate(),
    { login, signup: sign } = useAuth();
  ...
  const submit = async (e) => {
    ...
    if (reset) {
      ...
      try {
        ...
        toast.error(
          "Unable to send reset link. Check the email and try again.",
        );
      } finally {
        setBusy(false);
      }
    ...
    try {
      if (signup) {
        await sign(
          form.email,
          form.password,
          `${form.first} ${form.last}`.trim(),
        );
      } else await login(form.email, form.password);
      toast.success(signup ? "Welcome to GlobeTrotter" : "Welcome back");
      nav("/app/dashboard");
    } catch (err) {
      toast.error(
        signup
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:block relative">
        <ImageWithFallback
          ...
          alt="Travelers exploring a mountain destination"
        ...
        <div className="absolute left-12 bottom-12 text-white max-w-md">
          ...
          <p className="mt-3 text-white/80">
            Make every trip count with a beautifully organized itinerary.
          </p>
        </div>
      </div>
      <div className="grid place-items-center p-6 py-10">
        <div className="w-full max-w-xl">
          ...
          {!reset && !signup && (
            <Link
              to="/reset"
              className="block text-sm text-accent font-bold mt-4"
            >
              Forgot your password?
            </Link>
          )}
